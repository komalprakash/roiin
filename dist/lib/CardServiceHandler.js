...
var CardServiceHandler = function CardServiceHandler(PaysafeApiClient) {
...
CardServiceHandler.prototype.authorize = function (auth, responseCallBack) {
  try {
    ...
    if (typeof responseCallBack === "function") {
      var requestObj = new PaysafeRequest(prepareURI(AUTH_PATH, clientObj), constants.POST);
      ...
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : authorize");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (auth && auth.id) {
        ...
        var requestObj = new PaysafeRequest(prepareURI(AUTH_PATH + authId, clientObj), constants.PUT);
        clientObj.processRequest(requestObj, auth, function (error, response) {
          response = response ? new clientObj.Authorization(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : Auth id is missing " + "in CardServiceHandler : approveHeldAuth");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : approveHeldAuth");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (auth && auth.id) {
        ...
        throw clientObj.error(400, "InvalidRequestException : Auth id is missing " + "in CardServiceHandler : cancelHeldAuth");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : cancelHeldAuth");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
...
CardServiceHandler.prototype.reverseAuth = function (authReversal, responseCallBack) {
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (authReversal && authReversal.authorization && authReversal.authorization.id) {
        var authId = authReversal.authorization.id;
        delete authReversal.authorization;
        var requestObj = new PaysafeRequest(prepareURI(AUTH_PATH + authId + REVERSAUTH, clientObj), constants.POST);
        clientObj.processRequest(requestObj, authReversal, function (error, response) {
          response = response ? new clientObj.AuthorizationReversal(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : auth id is missing " + "in CardServiceHandler : reverseAuth");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : reverseAuth");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (settle && settle.authorization && settle.authorization.id) {
        ...
        var requestObj = new PaysafeRequest(prepareURI(AUTH_PATH + authId + SETTLEMENTS, clientObj), constants.POST);
        ...
        throw clientObj.error(400, "InvalidRequestException : Auth id is missing " + "in CardServiceHandler : settlement");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : settlement");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (settle && settle.id) {
        ...
        var requestObj = new PaysafeRequest(prepareURI(SETTLEMENTS + "/" + settlementId, clientObj), constants.PUT);
        clientObj.processRequest(requestObj, settle, function (error, response) {
          response = response ? new clientObj.Settlements(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : settlement id is missing " + "in CardServiceHandler : cancelSettlement");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : cancelSettlement");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (refund && refund.getSettlements() && refund.getSettlements().getId()) {
        ...
        var requestObj = new PaysafeRequest(prepareURI(SETTLEMENTS + "/" + settleId + REFUNDS, clientObj), constants.POST);
        ...
        throw clientObj.error(400, "InvalidRequestException : settlement id is missing " + "in CardServiceHandler : refund");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : refund");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (refund && refund.id) {
        ...
        var requestObj = new PaysafeRequest(prepareURI(REFUNDS + "/" + refundId, clientObj), constants.PUT);
        clientObj.processRequest(requestObj, refund, function (error, response) {
          response = response ? new clientObj.Refund(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : settlement id is missing " + "in CardServiceHandler : cancelRefund");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : cancelRefund");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (auth && auth.id) {
        var authId = auth.id;
        delete auth.id;
        var PaysafeRequestObj = new PaysafeRequest(prepareURI(AUTH_PATH + authId, clientObj), constants.GET);
        clientObj.processRequest(PaysafeRequestObj, null, function (error, response) {
          response = response ? new clientObj.Authorization(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : Auth id is missing " + "in CardServiceHandler : getAuth");
      }
    } else {
      console.error("Please provide the responseCallBack function " + "in CardServiceHandler : getAuth");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (authReversal && authReversal.id) {
        ...
        var requestObj = new PaysafeRequest(prepareURI(REVERSAUTH + authId, clientObj), constants.GET);
        clientObj.processRequest(requestObj, null, function (error, response) {
          response = response ? new clientObj.AuthorizationReversal(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : Reverse Auth id is missing in" + " CardServiceHandler : getAuthReversal");
      }
    } else {
      console.error("Please provide the responseCallBack function in" + " CardServiceHandler : getAuthReversal");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
    ...
    if (typeof responseCallBack === "function") {
      if (classObj && classObj.merchantRefNum) {
        ...
        if (upperClassName) {
          ...
          var requestObj = new PaysafeRequest(prepareURI(upperClassName + "?" + toInclude, clientObj), constants.GET);
          clientObj.processRequest(requestObj, null, function (error, response) {
            response = response ? new clientObj[className](response) : response;
            responseCallBack(error, response);
          });
        } else {
          throw clientObj.error(400, "InvalidClassException : " + "Please provide valid class name for search");
        }
      } else {
        throw clientObj.error(400, "InvalidRequestException : " + "Please provide merchant ref number for search");
      }
    } else {
      console.error("Please provide the responseCallBack function in" + " CardServiceHandler : searchByMerchantRef");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (settle && settle.id) {
        var settlementId = settle.id;
        delete settle.id;
        var requestObj = new PaysafeRequest(prepareURI(SETTLEMENTS + "/" + settlementId, clientObj), constants.GET);
        clientObj.processRequest(requestObj, null, function (error, response) {
          response = response ? new clientObj.Settlements(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : settlement id is missing in" + " CardServiceHandler : getSettlement");
      }
    } else {
      console.error("Please provide the responseCallBack function in" + " CardServiceHandler : getSettlement");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (refund && refund.id) {
        var refundId = refund.id;
        delete refund.id;
        var requestObj = new PaysafeRequest(prepareURI(REFUNDS + "/" + refundId, clientObj), constants.GET);
        clientObj.processRequest(requestObj, null, function (error, response) {
          response = response ? new clientObj.Refund(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : refund id is missing in" + " CardServiceHandler : CancelRefund");
      }
    } else {
      console.error("Please provide the responseCallBack function in" + " CardServiceHandler : CancelRefund");
    }
...
CardServiceHandler.prototype.verify = function (verification, responseCallBack) {
  try {
    ...
    if (typeof responseCallBack === "function") {
      var requestObj = new PaysafeRequest(prepareURI(VERIFICATION, clientObj), constants.POST);
      ...
      console.error("Please provide the responseCallBack function in" + " CardServiceHandler : verify");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
  ...
  try {
    ...
    if (typeof responseCallBack === "function") {
      if (verification && verification.id) {
        ...
        var requestObj = new PaysafeRequest(prepareURI(VERIFICATION + "/" + verificationId, clientObj), constants.GET);
        clientObj.processRequest(requestObj, null, function (error, response) {
          response = response ? new clientObj.Verification(response) : response;
          responseCallBack(error, response);
        });
      } else {
        throw clientObj.error(400, "InvalidRequestException : verification id is missing in" + " CardServiceHandler : getVerification");
      }
    } else {
      console.error("Please provide the responseCallBack function in" + " CardServiceHandler : getVerification");
    }
  } catch (err) {
    responseCallBack(err, null);
  }
